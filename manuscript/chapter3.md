...
У монтирования компонента есть два метода жизненного цикла: `componentWillMount()` и `componentDidMount()`. Сначала вызывается конструктор, метод `componentWillMount()` вызывается до метода `render()` и `componentDidMount()` вызывается после метода `render()`.
...
* **componentWillMount()** - вызывается перед методом жизненного цикла `render()`. По этой причине его можно использовать для установка внутреннего состояния компонента, поскольку он не будет вызывать вторую отрисовку компонента. Как правило, рекомендуется использовать  `constructor()` для установки начального состояния.
...
* **componentWillReceiveProps(nextProps)** - метод жизненного цикла вызывается во время обновления жизненного цикла. В качестве входных параметров вы получаете следующие свойства. Вы можете сравнить следующие свойства с предыдущими, используя `this.props`, чтобы применить разное поведение в зависимости от этого сравнения. Кроме того, вы можете установить состояние на основе следующих свойств.
...
* **componentWillUpdate(nextProps, nextState)** - вызывается немедленно до метода `render()`. В качестве входных параметров у вас уже есть следующие свойства и следующее состояние at your disposal. Вы можете использовать метод в качестве последней возмозможности выполнения подготовки до того, как метод отрисовки будет выполнен. Обратите внимание, что вы больше не можете вызывать `setState()`. Если вы хотите вычислить состояние на основе следующих свойств, вам нужно использовать `componentWillReceiveProps()`.

* **componentDidUpdate(prevProps, prevState)** - вызывается сразу после метода `render()`. Вы можете использовать как возможность выполнять операции с DOM или выполнять асинхронные запросы.
